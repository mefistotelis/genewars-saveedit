 ...
 * Simple wrapper for use with both little and big endian files.
 */
inline void write_int8_buf (unsigned char *buff, unsigned char x)
{
    buff[0]=x;
}

/**
 * Reads 1-byte number from given file.
 * Simple wrapper for use with both little and big endian files.
 */
inline unsigned char read_int8_file (FILE *fp)
{
    return fgetc(fp);
}

/**
 * Writes 1-byte number into given file.
 * Simple wrapper for use with both little and big endian files.
 */
inline void write_int8_file (FILE *fp, unsigned char x)
{
    fputc(x,fp);
}
